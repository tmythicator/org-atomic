...
REPEATER is an optional repeater string."
  (org-entry-put
   nil
   "SCHEDULED"
   (org-atomic-util-format-scheduled-timestamp day repeater)))
...
the past to the current day (or the next active day)."
  (when (and org-atomic-mode (not org-atomic--in-rollover))
    (let ((org-atomic--in-rollover t)
          (buffers (org-atomic-core--agenda-buffers t)))
      (dolist (buf buffers)
...
(defun org-atomic--roll-over-single-habit ()
  "Roll over the single habit at point if it is overdue."
  (let* ((habit-struct (org-atomic-core-parse-habit))
    ...
    (when (and habit-struct
      ...
      (let* ((time
             ...
             (sched-day (time-to-days time))
             (today (org-today)))
        (when (< sched-day today)
          (org-atomic--reschedule-to-active habit-struct today))))))

(defun org-atomic--refresh-agenda ()
  "Safely refresh the Org Agenda buffer if it is currently visible."
  (when (and (bound-and-true-p org-agenda-buffer-name)
